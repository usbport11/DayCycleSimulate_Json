...
bool MObjectBuffer::AddObject(MObject* Object)
{
	if(Id < 0)
	{
		LogFile<<"ObjectBuffer: Not initialized"<<endl;
	...
	if(!Object->GetTextureId())
	{
		LogFile<<"ObjectBuffer: ObjectBuffer: Null texture"<<endl;
		return false;
	}
	UVQuads.push_back(Object->Get());
	if(!Object->SetBufferOffset(ObjectsCount))
	{
		LogFile<<"ObjectBuffer: Can't set object offset"<<endl;
		return false;
...
bool MObjectBuffer::UpdateObject(MObject* Object, float pX0, float pY0, float pW, float pH, float tX0, float tY0, float tW, float tH, unsigned int inTextureId)
{
