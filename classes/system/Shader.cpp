...
bool MShader::PrepareShaderValues()
{
	...
	if(LightTextureId == -1)
	{
		LogFile<<"Shader: Can't get light map uniform"<<endl;
		return false;
	}
	...
	{
		LogFile<<"Shader: Can't get useoffset vertex uniform"<<endl;
		return false;
	}
	UseOffsetUVId = glGetUniformLocation(ProgramId, "useOffsetUV");
	if(UseOffsetUVId == -1)
	{
		LogFile<<"Shader: Can't get useoffset uv uniform"<<endl;
		return false;
	}
	AmbientColorId = glGetUniformLocation(ProgramId, "ambientColor");
	if(AmbientColorId == -1)
	{
		LogFile<<"Shader: Can't get ambient color uniform"<<endl;
		return false;
	}
	ResolutionId = glGetUniformLocation(ProgramId, "resolution");
	if(ResolutionId == -1)
	{
		LogFile<<"Shader: Can't get resolution uniform"<<endl;
		return false;
	}
